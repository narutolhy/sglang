...
class KVArgsRegisterInfo:
    ...
    # for staging buffer based heterogeneous TP transfer (dynamic allocator)
    dst_staging_base_ptr: int = 0
    dst_staging_total_size: int = 0
...
class MooncakeKVManager(CommonKVManager):
    ...
    def __init__(
        ...
        if self.disaggregation_mode == DisaggregationMode.PREFILL:
            ...
            self.remote_watermarks = (
                {}
            )  # session_id → (round, tail_offset) per decode rank
            ...
            self._room_bootstrap = {}
            self._room_receivers = {}
            if self.enable_staging:
                self._init_staging_allocator()
            self.start_decode_thread()

    def register_room_bootstrap(self, room, bootstrap_infos, receiver):
        """Store bootstrap connections and receiver for on-demand staging allocation."""
        self._room_bootstrap[room] = bootstrap_infos
        self._room_receivers[room] = receiver
    ...
    def _handle_staging_req(self, msg):
        ...
        receiver = self._room_receivers.get(room)
        if receiver is None:
            return
        ...
        if chunk_idx < len(infos) and infos[chunk_idx][0] >= 0:
            ...
            prefill_attn_tp = getattr(self, "prefill_attn_tp_size", attn_tp_size)
            total_kv_heads = resolve_total_kv_heads(
                self.kv_args,
                attn_tp_size,
                kv_buffer_tensors=getattr(self, "kv_buffer_tensors", None),
            )
            ...
            dst_tp_rank = self.kv_args.engine_rank % max(1, attn_tp_size)
    ...
    def _init_staging_buffers(self, count: int):
        ...
        for i in range(count):
            ...
            self.engine.batch_register([buf.get_ptr()], [buf.get_size()])
            self.staging_buffers.append(buf)
    ...
    def _init_staging_allocator(self):
        ...
        self.staging_allocator = StagingAllocator(
            pool_size_bytes,
            device,
            gpu_id,
            custom_mem_pool,
        ...
        self.engine.batch_register(
            [self.staging_allocator.get_base_ptr()],
            [self.staging_allocator.get_total_size()],
        )
        self.kv_buffer_tensors = None

    def set_kv_buffer_tensors(self, k_buffers: list, v_buffers: list, page_size: int):
        """Store references to the actual KV pool tensors for GPU gather/scatter."""
                ...
                for req in reqs_to_be_processed:
                    if not req.is_dummy:
                        ...
                        if tp_match:
                            ...
                            chunk_idx = (
                                kv_chunk.index_slice.start // full_chunk_pages
                            ...
                            if _STAGING_DEBUG:
                                ...
                                _rdma_elapsed = (
                                    _t["after_rdma"] - _t["before_rdma"]
                                ) * 1000
                                _wait_elapsed = (
                                    _t["after_rdma"] - kv_chunk._enqueue_time
                                ) * 1000
                                if ret == 0:
            ...
            while True:
                ...
                if room == "WATERMARK":
                    ...
                    with self._watermark_cv:
                        ...
                        is_regression = (wm_round < prev[0]) or (
                            wm_round == prev[0] and wm_tail < prev[1]
                        )
                        if is_regression:
                            logger.warning(
                    ...
                    dst_tp = (
                        getattr(decode_info, "dst_tp_rank", "?") if decode_info else "?"
                    )
                    if _STAGING_DEBUG:
                        logger.info(
                ...
                if room == "STAGING_RSP":
                    ...
                    if tinfo is not None:
                        while len(tinfo.staging_offsets) <= stg_chunk_idx:
                            tinfo.staging_offsets.append(-1)
                            tinfo.staging_rounds.append(0)
                            tinfo.staging_ends.append(-1)
        ...
        def decode_thread():
            while True:
                ...
                if msg[0] == b"CHUNK_READY":
                    ...
                    prefill_rank = msg[6].decode("ascii") if len(msg) > 6 else "?"
                    ...
                    group_len = sum(
                        1
                        for c in self.pending_chunk_scatters[room]
                        if c[0] == chunk_idx
                    )
                    if _STAGING_DEBUG:
                        logger.info(
...
class MooncakeKVReceiver(CommonKVReceiver):
    ...
    def _register_kv_args(self):
        for bootstrap_info in self.bootstrap_infos:
            ...
            if staging_allocator is not None:
                packed_staging_base_ptr = struct.pack(
                    "Q", staging_allocator.get_base_ptr()
                )
                staging_total_size_str = str(staging_allocator.get_total_size()).encode(
                    "ascii"
                )
            else:
    ...
    def _allocate_chunk_staging(self, kv_indices):
        ...
        chunked_prefill_size = (
            getattr(self.kv_mgr, "prefill_chunked_prefill_size", None)
            or self.kv_mgr.server_args.chunked_prefill_size
            or 8192
        )
        ...
        if _STAGING_DEBUG:
            logger.info(
                ...
                *post_alloc_state,
                rounds_used,
                *staging_allocator.get_watermark(),
            )
    ...
    def init(
        ...
        offsets_str = (
            ",".join(str(info[1]) for info in self.chunk_staging_infos)
            if self.chunk_staging_infos
            else "-1"
        )
        rounds_str = (
            ",".join(str(info[2]) for info in self.chunk_staging_infos)
            if self.chunk_staging_infos
            else "0"
        )
        ends_str = (
            ",".join(str(info[3]) for info in self.chunk_staging_infos)
            if self.chunk_staging_infos
            else "-1"
        )
